...
public class BuyGasTest extends base {
	...
	public void buyEnergyTest(String data) throws InterruptedException, IOException {
		...
		int availquantity = Integer.parseInt(bepo.availQty().getText());
		if (quantity > 0 && quantity <= availquantity) {
			bepo.supplyUnitsToBuy().sendKeys(data);
			bepo.buyButton().click();
			SaleConfirmationPageObject scpo = new SaleConfirmationPageObject(driver);
			String saleConfmsg = scpo.confirmSale().getText();
			Assert.assertEquals(saleConfmsg, "Sale Confirmed!");
			Thread.sleep(1000);
			File src = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
			FileUtils.copyFile(src, new File(System.getProperty("user.dir") + "\\Screenshots\\SaleConfirmation.png"));
			scpo.goback().click();
		} else {
	...
	public Object[][] dpMethod() {
		return new Object[][] { { "2000" }, { "4000" }, { "0" }, { "-1" } };
	}
